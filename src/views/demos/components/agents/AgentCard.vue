<template>
	...
	<div class="d-flex">
		<div class="agent-list-image">
			<RouterLink v-if="agent.profile" :to="{name : 'demo01.agent-details', params: { agentUsername: agent.username}}">
		...
		<div class="agent-list-content flex-grow-1">
			<div class="d-flex xxs-column">
				<h2>
					<!-- <span class="badge badge-success agent-verified-badge"><i class="houzez-icon icon-check-circle-1 mr-1"></i> Verified</span>  -->
					 <RouterLink :to="{name : 'demo01.agent-details', params: { agentUsername: agent.username}}"> {{ agent.name }} </RouterLink>
				</h2>
				<!-- <?php include 'inc/property/templates/rating.php';?> -->
			</div><!-- d-flex -->
			<p class="agent-list-position"> {{ agent.position || null }} <a href="#"><!--  Modern House Real Estate --></a></p>
			<ul class="agent-list-contact list-unstyled">
				<li><strong>Office</strong> <span class="agent-phone agent-phone-hidden">{{ agent.phone || null }}</span></li>
				<li><strong>Mobile</strong> <span class="agent-phone agent-phone-hidden">{{ agent.mobile || null }}</span></li>
			...
			<div class="d-flex sm-column">
				...
				<RouterLink :to="{name : 'demo01.agent-details', params: { agentUsername: agent.username}}" class="agent-list-link mt-3"><strong>View My Listings</strong></RouterLink>
			</div><!-- d-flex -->
