...
const props = defineProps(['leads', 'leadsLoading'])

const leadsSummary = computed(() => [
  {
    label: 'Last 24 Hours',
    count: props.leads?.last_24_hours?.count ?? 0,
    change: props.leads?.last_24_hours?.change ?? 0
  },
  {
    label: 'Last 7 Days',
    count: props.leads?.last_7_days?.count ?? 0,
    change: props.leads?.last_7_days?.change ?? 0
  },
  {
    label: 'Last 30 Days',
    count: props.leads?.last_30_days?.count ?? 0,
    change: props.leads?.last_30_days?.change ?? 0
  }
])
