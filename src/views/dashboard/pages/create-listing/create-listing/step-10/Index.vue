...
// form state (string for radio binding)
const formData = ref({
  contact_information: '',
});
...
// Normalize DB value (["agent_data"] → "agent_data")
...
onMounted(() => {
  ...
  if (property.value) {
    formData.value = {
      ...property.value,
      contact_information: normalizeContactInfo(property.value.contact_information),
    };
  }
});
...
watch(property, (newProperty) => {
  if (newProperty) {
    formData.value = {
      ...newProperty,
      contact_information: normalizeContactInfo(newProperty.contact_information),
    };
  }
});
</script>
